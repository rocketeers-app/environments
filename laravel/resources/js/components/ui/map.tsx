...
// which under a bundler points at the app chunk, not the worker file, so the
// worker 404s. Bundler consumers hand it the URL once; `?worker&url` makes Vite
// emit a self-contained worker chunk (plain `?url` drops its shared sibling).
...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  useEffect(() => {
    ...
    const map = new MapLibreGL.Map({
      ...
      // Attribution control hidden by default. Note: CARTO/OpenStreetMap terms
      // require visible attribution for commercial use — re-enable per-map if needed.
